...
class Reader:
    ...
    def extract_text(self, img, show_text=False, show_confidence=False):
        ...
        for text in filter(lambda x: x[-1] > 0.45, result):
            ...
            img = cv2.rectangle(
                img, [int(i) for i in box[0]], [int(i) for i in box[2]], (0, 255, 0), 2
            )  # each coordinate is a list has to be int
...
class GPT_3:
    ...
    def summarize(self, text):
        prompt = f"Try to summarize the following text as best as you can!\n\n{text}"
...
if __name__ == "__main__":
    ...
    img = read_img("./ScreenShot_20230717082047.png")
    text, extracted_img = reader(img)
